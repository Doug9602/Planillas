...
def calcular_monto_vacaciones(sueldo_base, fecha_ingreso: date, fecha_corte: date) -> float:
    if fecha_corte.month == fecha_ingreso.month:
        anios = calcular_anios(fecha_ingreso, fecha_corte)
        if anios >= 1:
...
# NUEVA FUNCIÓN: ISSS Patronal con tope
def calcular_isss_patronal(monto_cotizable) -> float:
    base = min(float(monto_cotizable), TOPE_ISSS)
    return base * ISSS_PATRONAL_PCT
